...
/**
 * Clase principal de prueba de algunas tablas de datos, estadísticas y de
 * análisis
 * 
...
public class PruebaTablas {
	...
	private static void sacaVentanasPrueba() {
		try {
			...
			try {
				for (LookAndFeelInfo info : UIManager.getInstalledLookAndFeels()) {
					if ("Nimbus".equals(info.getName())) {
						...
						UIManager.getLookAndFeelDefaults().put("Table.focusCellHighlightBorder",
								new Insets(0, 0, 0, 0));
			...
			} // Si no está disponible nimbus, no se hace nada
			...
			for (int i = 0; i < tabla.getWidth(); i++)
				System.out.print(tabla.getType(i).getSimpleName() + "\t");
			System.out.println();

			// 3.- Crea una ventana general para visualizar las tablas y añade una ventana
			// interna con los datos de prueba
			...
			// 4.- Crea una segunda tabla estadística filtrada de la primera y la añade a
			// la ventana
			TablaEstadistica dataset1 = tabla.creaTablaFiltro("tipo", true, "DATASET1", null, "datoI", "datoD",
					"datoD2", "Par-Impar");
			...
			ArrayList<Color> lColores = new ArrayList<>(Arrays.asList(new Color(247, 202, 121), new Color(146, 208, 80),
					new Color(255, 0, 0), new Color(39, 228, 253), Color.YELLOW));
			ArrayList<Color> lColoresBorde = new ArrayList<>(
					Arrays.asList(Color.BLACK, Color.BLACK, Color.BLACK, Color.BLACK, Color.BLACK));
			...
			ta.setRenderer4Cuartiles(vDatos, true, "t1", "t5", Color.WHITE, new Color(170, 170, 255),
					new Color(90, 90, 255), Color.BLUE);
			ta.setRenderer4Cuartiles(vDatos, false, "p1", "p5", Color.RED, ROJO_CLARO, VERDE_CLARO, Color.GREEN);
			vDatos.setVisible(true);
			vDatos.pack();

			// 6.- Crea una tabla de análisis de medias de tiempos (por equipos) desde la
			// tabla de datos
			...
			// 7.- Crea una tabla de análisis de suma de puntos (por equipos) de desde la
			// tabla de datos
			...
			VentanaTabla.EventoEnCelda ev = new VentanaTabla.EventoEnCelda() {
				...
				public void evento(VentanaTabla tabla, int fila, int columna) {
					if (columna < 2)
						return; // Solo se pueden editar las columnas 2 en adelante
					Tabla t = tabla.getTabla();
					Class<?> tipoDato = t.getTipos().get(columna);
					Object o = t.get(fila, columna);
					String valAnterior = "";
					if (o != null)
						valAnterior = o.toString();
					String nuevoVal = (String) JOptionPane.showInternalInputDialog(tabla, "Introduce nuevo valor:",
							"Cambio de dato " + t.getCabecera(columna), JOptionPane.QUESTION_MESSAGE, null, null,
							valAnterior);
					if (nuevoVal != null) {
						if (tipoDato == String.class) {
							...
							try {
								...
								JOptionPane.showInternalMessageDialog(tabla, "Valor introducido incorrecto:" + nuevoVal,
			...
			// T2
			vgt.addMenuAccion("Ejercicio 2", new ActionListener() {

				@Override
				public void actionPerformed(ActionEvent e) {
					// Opcion 1
					// sortConArrayList(ta);
					// Opcion 2 -> Para hacer esta opcion tenemos que hacer comparable la clase Puntuacion
					Map<Integer, TreeSet<Puntuacion>> puntuaciones = new HashMap<>();
					for (int fila = 0; fila < ta.size(); fila++) {
						String nick = (String) ta.get(fila, "nick");
						// Creamos el objeto puntuacion
						for (int columna = 1; columna <= 5; columna++) {
							int p = 0;
							double t = 99;
							if (ta.get(fila, "t" + columna) != null) {
								p = (int) ta.get(fila, "p" + columna);
							}

							if (ta.get(fila, "p" + columna) != null) {
								t = (double) ta.get(fila, "t" + columna);
							}
							Puntuacion puntuacion = new Puntuacion(nick, t, p);
							// Si la lista no existe
							if (!puntuaciones.containsKey(columna)) {
								puntuaciones.put(columna, new TreeSet<>());
							}
							// Obtenemos la lista y a�adimos el objeto
							puntuaciones.get(columna).add(puntuacion);
						}

						for (Entry<Integer, TreeSet<Puntuacion>> clasificaion : puntuaciones.entrySet()) {
							TreeSet<Puntuacion> clasificacionNivel = clasificaion.getValue();
							System.out.println("Clasificacion nivel: " + clasificaion.getKey());
							for (Puntuacion puntuacion : clasificacionNivel) {
								System.out.println(puntuacion.getNick() + " Puntos: " + puntuacion.getP() + " Tiempo: "
										+ puntuacion.getT());
							}
						}
					}
				...
				private void sortConArrayList(TablaEstadistica ta) {
					Map<Integer, ArrayList<Puntuacion>> puntuaciones = new HashMap<>();
					for (int fila = 0; fila < ta.size(); fila++) {
						String nick = (String) ta.get(fila, "nick");
						// Creamos el objeto puntuacion
						for (int columna = 1; columna <= 5; columna++) {
							...
							// Si la lista no existe
							if (!puntuaciones.containsKey(columna)) {
								puntuaciones.put(columna, new ArrayList<>());
							}
							// Obtenemos la lista y a�adimos el objeto
							puntuaciones.get(columna).add(puntuacion);
						}
						Comparator<Puntuacion> comparador = new Comparator<Puntuacion>() {
						...
						for (Entry<Integer, ArrayList<Puntuacion>> clasificaion : puntuaciones.entrySet()) {
							ArrayList<Puntuacion> clasificacionNivel = clasificaion.getValue();
							clasificacionNivel.sort(comparador);
							System.out.println("Clasificacion nivel: " + clasificaion.getKey());
							for (Puntuacion puntuacion : clasificacionNivel) {
								System.out.println(puntuacion.getNick() + " Puntos: " + puntuacion.getP() + " Tiempo: "
										+ puntuacion.getT());
	...
	// Método llamado tras cambiar el valor de la tabla con doble click y un valor
	// correcto
